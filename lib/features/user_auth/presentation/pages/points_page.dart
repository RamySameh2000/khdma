...
class _PointScreenState extends State<PointScreen> {
  final List<Activity> activities = [
    ...
    Activity(id: DateTime.now().millisecondsSinceEpoch.toString(), userEmail: '', name: 'إصحاح من الأنجبل', points: 1, timestamp: DateTime.now(), userName: ''),
  ];
                    ...
                    if (selectedActivity != null || (_activityController.text.isNotEmpty && points > 0)) {
                      ...
                      if (user != null) {
                        final activity = Activity(
                          ...
                          isApproved: false,
                          userName: _userName, // Set the userName here
                        );
    ...
    if (user != null) {
      try {
        await firestore.collection('requests').add({
          'userEmail': user.email,
          'userName': activity.userName, // Include userName in the request
          'name': activity.name,
